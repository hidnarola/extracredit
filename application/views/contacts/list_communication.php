...
<div class="page-header page-header-default">
    <div class="page-header-content">
        <div class="page-title">
            <h4><i class="icon-comment-discussion"></i> <span class="text-semibold"><?php echo $contact['name'] ?> Communication</span></h4>
        </div>
    </div>
    <div class="breadcrumb-line">
        <ul class="breadcrumb">
            ...
            <li class="active">Contact Communication</li>
        </ul>
    </div>
</div>
<div class="content">
    ...
    <div class="panel panel-flat">
        <div class="panel-heading text-right">
            <a href="<?php echo site_url('contacts/add_communication' . '/' . $id); ?>" class="btn btn-success btn-labeled"><b><i class="icon-plus-circle2"></i></b> Add Conversation</a>
        </div>
...
<script>
    ...
    $(function () {
        $('.datatable-basic').dataTable({
            ...
            order: [[6, "desc"]],
            ajax: site_url + 'contacts/get_contacts_communication/' + contact_id,
            columns: [
                {
                    data: "sr_no",
                    visible: true,
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if ($.inArray('edit', permissions) !== -1) {
                            action += '<a href="' + site_url + 'contacts/add_communication/' + btoa(full.contact_id) + '/' + btoa(full.id) + '" class="btn border-primary text-primary-600 btn-flat btn-icon btn-rounded btn-xs" title="Edit Communication"><i class="icon-pencil3"></i></a>';
                        }
                        if ($.inArray('view', permissions) !== -1) {
                            action += '&nbsp;&nbsp;<a href="javascript:void(0)"  data-toggle="modal" data-target="#modalviewConversation" class="btn border-purple text-purple-600 btn-flat btn-icon btn-rounded btn-xs" data-id=' + btoa(full.id) + ' onclick="return view_communication(this)" title="View Communication"><i class="icon-eye"></i></a>'
                        }
                        if ($.inArray('delete', permissions) !== -1) {
                            action += '&nbsp;&nbsp;<a href="' + site_url + 'contacts/delete_communication/' + btoa(full.contact_id) + '/' + btoa(full.id) + '" class="btn border-danger text-danger-600 btn-flat btn-icon btn-rounded btn-xs" onclick="return confirm_alert(this)" title="Delete Communication"><i class="icon-trash"></i></a>'
                        }
                        return action;
                    }
                }
    ...
    function view_communication(e) {
        var url = site_url + 'contacts/get_communication_by_id';
    ...
    function confirm_alert(e) {
        swal({
            title: "Are you sure?",
            text: "You will not be able to recover this contact communication!",
            type: "warning",
