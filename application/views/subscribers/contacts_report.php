...
<div class="page-header page-header-default">
    <div class="page-header-content">
        <div class="page-title">
            <h4><i class="icon-coins"></i> <span class="text-semibold">Contacts Report</span></h4>
        </div>
    </div>
    <div class="breadcrumb-line">
        <ul class="breadcrumb">
            <li><a href="<?php echo site_url('home'); ?>"><i class="icon-home2 position-left"></i> Home</a></li>
            <li class="active">Contacts Report</li>
        </ul>
    </div>
</div>
<div class="content">
    ...
    <div class="panel panel-flat custom_perpage_dropdown">
        ...
        <table class="table"  id="contacts_report">
            <thead>
                <tr>
                    <th>Email</th>
                    <th>Contact Type</th>
            ...
            <?php foreach($contacts as $row) {?>
                <tr>                       
                    ...
                    <td><?php if(isset($row['type']) and $row['type'] != ''){echo $row['type'];}else{echo '-';} ?></td>
                    <td><?php if(isset($row['name']) and $row['name'] != ''){echo $row['name'];}else{echo '-';} ?></td>
                    <td><?php if(isset($row['address']) and $row['address'] != ''){echo $row['address'];}else{echo '-';} ?></td>
                    <td><?php if(isset($row['created']) and $row['created'] != ''){echo $row['created'];}else{echo '-';} ?></td>
                    <td><?php if(isset($row['state']) and $row['state'] != ''){echo $row['state'];}else{echo '-';} ?></td>
                    <td><?php if(isset($row['city']) and $row['city'] != ''){echo $row['city'];}else{echo '-';} ?></td>
                    <td><?php if(isset($row['zip']) and $row['zip'] != ''){echo $row['zip'];}else{echo '-';} ?></td>
                </tr>
            <?php } ?>
            </tbody>
        </table>
